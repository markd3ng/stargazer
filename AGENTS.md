...
## Upstream Merge Protocol (Rule 14)

本项目是 [`mihomo-party-org/clash-party`](https://github.com/mihomo-party-org/clash-party) 的 fork + rebrand（Clash Party → Star Gazer）。

### 合并原则

- **永远 `git merge`，禁止 `git rebase`** — 分支有大量重命名提交，rebase 会重复解冲突
- 合并时保护 **Fork DNA**（见下文），禁止上游覆盖分支的永久差异
- 合并后走完整验证流程（typecheck + lint + build）

### Fork DNA（合并时必须保留的分支差异）

1. **重命名**：Clash Party → Star Gazer（窗口标题、locale、gist 描述、UA、URL、Bundle ID）
2. **已移除**：Sub-Store、Telegram 通知、交互式教程（driver.js）
3. **分支独有功能**：ICMP 转发禁用、IPv6 fake-ip-range、证书指纹、Tailscale 通知、代理 Tooltip
4. **CI/CD**：仅 macOS arm64 DMG + Windows x64/arm64 + Linux

### 完整流程

详见 **[`docs/rules/upstream_merge.md`](docs/rules/upstream_merge.md)** — 包含冲突分类表、逐文件解法、验证清单。
